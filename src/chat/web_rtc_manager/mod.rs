...
use web_sys::{
    console, RtcDataChannel, RtcDataChannelEvent, RtcDataChannelInit, RtcIceCandidate,
...
pub trait NetworkManager {
    ...
    fn validate_offer(web_rtc_manager: Rc<RefCell<Self>>, str: &str) -> Result<(), RTCError>;
    fn validate_answer(web_rtc_manager: Rc<RefCell<Self>>, str: &str) -> Result<(), RTCError>;
...
impl NetworkManager for WebRTCManager {
    ...
    fn send_message(&self, message_content: &str) {
        if let Some(data_channel) = &self.data_channel {
            ...
            console::error_1(&"Data channel is not available".into());
        }
    }
    ...
    fn validate_offer(
        ...
        let remote_description_js_value: JsValue =
            JSON::parse(&connection_string.offer).map_err(|_| RTCError::InvalidOffer)?;
        ...
        let create_answer_function: Box<dyn FnMut(JsValue)> = Box::new(move |a: JsValue| {
            ...
            let create_answer_exception_handler =
                Closure::wrap(Box::new(move |_send_channel: JsValue| {
                    ...
                    console::error_1(&"Exception in create_answer".into());
                    console::error_1(&a);
...
impl WebRTCManager {
    fn set_candidates(
        ...
        for candidate in &connection_string.ice_candidates {
            ...
            ice_candidate_init.set_candidate(&candidate.candidate);
            ice_candidate_init.set_sdp_mid(Some(&candidate.sdp_mid));
            ice_candidate_init.set_sdp_m_line_index(Some(candidate.sdp_m_line_index));
    ...
    fn parse_base64_str_to_connection(str: &str) -> Result<ConnectionString, RTCError> {
        BASE64_STANDARD
            .decode(str)
            .map_err(RTCError::InvalidBase64)
            .and_then(|a| {
                str::from_utf8(&a)
                    .map(|s| s.to_string())
                    .map_err(RTCError::InvalidString)
            })
            .and_then(|a_str| {
                serde_json::from_str::<ConnectionString>(&a_str)
                    .map_err(RTCError::SerializationError)
            })
            .and_then(|connection_string| {
                JSON::parse(&connection_string.offer)
                    .map_err(|_| RTCError::InvalidOffer)
                    .map(|_| connection_string)
            })
    }
